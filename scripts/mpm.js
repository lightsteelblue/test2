...
export const init = (gl) => {
    ...
    _P2GProgram = new ShaderProgram(_gl, _vertSources[0], _fragSources[0], [0, 1, 2], [4, 4, 1]);
    _G2PProgram = new ShaderProgramTF(_gl, _vertSources[2], _fragSources[2], ['o_posVel', 'o_C', 'o_J'], [0, 1, 2], [4, 4, 1]);
    ...
    for (let y = 0; y < gridY / 2; y+=_dp)
        for (let x = 0; x < gridX / 2; x+=_dp) {
            pos.push(x + 0.*Math.random() + 2.5);
            pos.push(y + 0.*Math.random() + 2.5);
        }
    }
...
export const render = (renderer) => {
    ...
    _gl.uniform2f(_drawProgram.uniform('canvasSize'), 960, 640);
